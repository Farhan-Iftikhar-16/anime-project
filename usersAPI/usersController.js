...
const favouriteAnime = require('./userFavouriteAnime');
const comments = require('./userCommentsModel');
const rating = require('./animeRatingModel');
...
module.exports = {
  ...
  addFavorite: (req, res) => {
    ...
    const user_id = req.signedCookies.userId;
    favouriteAnime.addFavourite(anime_id, user_id, (err) => {
      if (err) {
        res.status(err.status || 500).render('error', {title: 'Ошибка', userEntered: req.signedCookies.userId, message: `Код ошибки: ${err.status || 500}`});
      } else {
  ...
  viewedFavorite: (req, res) => {
    const anime_id = req.params.anime_id;
    const user_id = req.signedCookies.userId;
    favouriteAnime.viewedFavorite(anime_id, user_id, (err, result) => {
      if (err) {
        res.status(err.status || 500).render('error', {title: 'Ошибка', userEntered: req.signedCookies.userId, message: `Код ошибки: ${err.status || 500}`});
      } else {
        res.redirect(`/users/${user_id}/favorites`)
  ...
  postComment: (req, res) => {
    ...
    comments.addComment(user_id, anime_id, user_comment, (err) => {
      if (err) {
        res.status(err.status || 500).render('error', {title: 'Ошибка', userEntered: req.signedCookies.userId, message: `Код ошибки: ${err.status || 500}`});
      } else {
  ...
  allYourComments: (req, res) => {
    const user_id = req.params.user_id;
    comments.getComments(user_id, (err, result) => {
      if (err) {
        res.status(err.status || 500).render('error', {title: 'Ошибка', userEntered: req.signedCookies.userId, message: `Код ошибки: ${err.status || 500}`});
      } else {
  ...
  deleteComment: (req, res) => {
    const body = req.body;
    const user_id = req.signedCookies.userId;
    comments.deleteComment(body, (err) => {
      if (err) {
        res.status(err.status || 500).render('error', {title: 'Ошибка', userEntered: req.signedCookies.userId, message: `Код ошибки: ${err.status || 500}`});
      } else {
  ...
  animeRatingHandler: (req, res) => {
    ...
    rating.addRating(anime_id, user_id, rating, (err) => {
      if (err) {
        res.status(err.status || 500).render('error', {title: 'Ошибка', userEntered: req.signedCookies.userId, message: `Код ошибки: ${err.status || 500}`});
      } else {
        res.redirect(`/anime/description/${anime_id}`);
